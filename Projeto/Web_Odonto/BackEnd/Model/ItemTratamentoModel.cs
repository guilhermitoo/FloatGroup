...
namespace BackEnd.Model
{
    public class ItemTratamentoModel
    {
        ...
        public List<itemTratamento> ListarPorTratamento(int idTratamento)
        {
            using (WebOdontoClassesDataContext db = new WebOdontoClassesDataContext())
            {

                String sSql = "select IT.* from itensTratamento IT " +                              
                              " where IT.tratamento_id =" + idTratamento.ToString();
